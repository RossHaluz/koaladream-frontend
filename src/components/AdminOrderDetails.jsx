...
const AdminOrderDetails = ({order}) => {
    const {postalService, city, department, payment, lastName, firstName, phone, email} = order;

  return     <div className='py-[30px] px-[15px]'>
  {order && <div className='p-[30px] border border-s border-[#7FAA84] rounded-[5px] flex flex-col gap-[30px]'>
...
<div className='flex flex-col gap-[15px]'>
    {order && order?.items.map(({image, title, article, id}) => {
